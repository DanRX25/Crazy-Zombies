...
public:
    ...
    void salto(RectangleShape& p1) { posY -= 100; p1.setPosition(posX, posY); }
...
bool DetectSuelo(T user, U object) {
    ...
    if ((is_same<T, Player>::value || is_same<T, Enemy>::value) && is_same<U, Obstaculo>::value) {
        if ((user.getPosY() + user.getAlto()) >= object.getPosY() && (user.getPosY() + user.getAlto()) <= (object.getPosY() + 1) && ancho)
            return true;
        return false;
    }
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Left)) {
            if (!(colision<Player, Obstaculo>(p1, o1) == -1 || colision<Player, Enemy>(p1, e1) == -1)) {
